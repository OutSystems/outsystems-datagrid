...
namespace OSFramework.Column {
    ...
    export abstract class AbstractColumn<T extends IConfigurationColumn>
         ...
         */
        constructor(grid: OSFramework.Grid.IGrid, columnID: string, configs: T) {
            this._grid = grid;
        ...
        private _findParentGroup(elem: Element): string {
            const group = elem.closest(OSFramework.Helper.Constants.columnGroupTag);

            if (group !== null) {
                return group.firstElementChild // Consider that the GenericColumn widget is the first element on GroupColumn
                    .querySelector(OSFramework.Helper.Constants.columnUniqueIdCss)
                    .getAttribute('name');
            }
        ...
        protected _preBuild(): void {
            ...
            if (!this._configs.autoGenerated) {
                const unique = OSFramework.Helper.GetElementByUniqueId(this._uniqueId);
        ...
        public indexPosition(): number {
            ...
            const gridElement = OSFramework.Helper.GetElementByWidgetId(
                this._grid.widgetId
            );
            const thisColumn = OSFramework.Helper.GetElementByWidgetId(this._widgetId);

            return _.toArray(
                gridElement.querySelectorAll(OSFramework.Helper.Constants.columnCss)
            )
                .map((p) => p.parentNode)
                .indexOf(thisColumn);
        }
