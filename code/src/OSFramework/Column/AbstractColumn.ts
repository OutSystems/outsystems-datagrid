...
namespace OSFramework.Column {
    ...
    > implements IColumn {
        ...
        private _findParentGroup(elem: Element): string {
            const group = elem.closest(
                OSFramework.Helper.Constants.columnGroupTag
            );

            if (group !== null) {
                return group.firstElementChild // Consider that the GenericColumn widget is the first element on GroupColumn
                    .querySelector(
                        OSFramework.Helper.Constants.columnUniqueIdCss
                    )
                    .getAttribute('name');
            }
        ...
        protected _preBuild(): void {
            ...
            if (!this._configs.autoGenerated) {
                const unique = OSFramework.Helper.GetElementByUniqueId(
                    this._uniqueId
                );
        ...
        public indexPosition(): number {
            ...
            const thisColumn = OSFramework.Helper.GetElementByWidgetId(
                this._widgetId
            );
