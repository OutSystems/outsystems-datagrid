...
namespace Features {
    ...
    class GridInsertRowAction extends wijmo.undo.UndoableAction {
        ...
        constructor(grid: wijmo.grid.FlexGrid, undoableItems: {datasourceIdx: number, items: any[]}) {
            super(grid);
        ...
        public applyState(state: any) {
            ...
            if (cv) {
                if(state.action === 'remove'){
                    ...
                    //redo
                    cv.sourceCollection.splice(state.datasourceIdx,0, ...state.items);
    ...
    export class Rows implements IBuilder, IRows {
        ...
        private _getTopRow(): number {
            const topRow = Math.min(...this._grid.features.selection.getAllSelections().map(cellRange => cellRange.topRowIndex));
        ...
        public addNewRows(): _ErrorMessage {
            if (!this._canAddRows()) {
                // Return error
                return {code: 201, message: 'It seems that you have an active filter, group or sort on your columns. Remove them and try again.' };
            }

            if(!this._newItem) {
                return { code: 400, message: 'If you use auto generated columns and JSONSerialize, you can\'t add new rows. Also, if you are using columns, JSONSerialize and the grid has no data, you can\'t add new rows.'};
            } 
            ...
            const dsTopRowIndex = topRowIndex + this._grid.features.pagination.rowStart - 1;
            // Consider the quantity 1 if there is no selection.
            const quantity = this._grid.features.selection.getSelectedRowsCountByCellRange() || 1;
            ...
            for (let i = quantity; i > 0; i--) {
                ...
                dataSource.deferUpdate(() => {
                    dataSource.sourceCollection.splice(dsTopRowIndex, 0, _newItem);
                });
            ...
            this._grid.features.undoStack.pushAction(new GridInsertRowAction(providerGrid, undoableItems));
        ...
        public removeSelectedRows(): _ErrorMessage {
            if (!this._canRemoveRows()) {
                return { code: 400, message: 'It seems that you have an active filter, group or sort on your columns. Remove them and try again.' };
            }
            ...
            const expectedRowCount = this._getRowsCount() - this._grid.features.selection.getSelectedRowsCountByCellRange();
            ...
            dataSource.deferUpdate(() => {
                selRanges.forEach(range => {
                    for (let row = range.bottomRow; row >= range.topRow; row--) {
                        ...
                        providerGrid.onDeletingRow(new wijmo.grid.CellRangeEventArgs(providerGrid.cells, new wijmo.grid.CellRange(row, -1)));
