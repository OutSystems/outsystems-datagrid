...
namespace Providers.DataGrid.Wijmo.Column {
		...
		implements OSFramework.DataGrid.Column.IColumnCustom
	{
		...
		constructor(grid: OSFramework.DataGrid.Grid.IGrid, columnID: string, configs: T, editorConfigs: K) {
		...
		public get hasConditionalFormat(): boolean {
			return this.editorConfig.conditionalFormat !== undefined && this.editorConfig.conditionalFormat.length > 0;
		}
		...
		public build(): void {
			...
			this._editor = this._editor
				? this._editor
				: new wijmoControl(document.createElement('div'), this._editorConfigs.getProviderConfig());
		...
		public changeProperty(propertyName: string, propertyValue: any): void {
			...
			if (this.editorConfig.hasOwnProperty(propertyName)) {
				...
				if (
					...
					propertyName === OSFramework.DataGrid.OSStructure.ColumnProperties.ConditionalFormat
				) {
