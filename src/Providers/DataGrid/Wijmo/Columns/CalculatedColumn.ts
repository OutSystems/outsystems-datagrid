	...
	export class CalculatedColumn extends AbstractProviderColumn<OSFramework.DataGrid.Configuration.Column.ColumnConfigAdditional> {
		constructor(
			...
			super(
				grid,
				columnID,
				new OSFramework.DataGrid.Configuration.Column.ColumnConfigAdditional(configs, extraConfig)
			);
			...
			this.config.binding =
				'$' + this.config.header.replace(/[^a-zA-Z_0-9-]/g, '') + '_' + extraConfig.formula.function;
		}
