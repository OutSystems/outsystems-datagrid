...
namespace Providers.DataGrid.Wijmo.Column {
	...
	> extends OSFramework.DataGrid.Column.AbstractColumn<T> {
		...
		public get columnEvents(): OSFramework.DataGrid.Event.Column.ColumnEventsManager {
			throw new Error(`The column ${this.columnType.toString()} does not support events`);
		}
		...
		protected _getVisibility(): boolean {
			...
			const inGroupPanel = this.grid.features.groupPanel.columnInGroupPanel(this.config.binding);
			...
			return (
				(providerConfig.visible && this.provider.isVisible && !inGroupPanel && !inColumnPicker) ||
				(providerConfig.visible && !this.provider.isVisible && !inGroupPanel && inColumnPicker)
			);
		}
		...
		public build(): void {
			...
			if (this.hasParentColumn) {
				const parent = this.grid.getColumn(this.parentColumnId) as OSFramework.DataGrid.Column.IColumnGroup;
				...
				this.provider = new wijmo.grid.ColumnGroup(this.getProviderConfig());
