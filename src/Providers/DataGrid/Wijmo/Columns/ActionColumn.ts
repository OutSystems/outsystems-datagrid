	...
	export class ActionColumn extends AbstractProviderColumn<OSFramework.DataGrid.Configuration.Column.ColumnConfigAction> {
		constructor(
			...
			super(
				grid,
				columnID,
				new OSFramework.DataGrid.Configuration.Column.ColumnConfigAction(configs, extraConfig)
			);
		...
		public handleActionEvent(ctx: wijmo.grid.ICellTemplateContext): void {
			...
			const clonedDataItem = OSFramework.DataGrid.Helper.DeepClone(ctx.item);
			...
			this._columnEvents.trigger(
				...
				JSON.stringify(
					this.grid.isSingleEntity ? OSFramework.DataGrid.Helper.Flatten(clonedDataItem) : clonedDataItem
				)
			);
		}
	}
