...
namespace Providers.DataGrid.Wijmo.Helper.FilterFactory {
	...
	function _createCondition(
		...
		const filterCondition = new OSFramework.DataGrid.OSStructure.FilterCondition();
	...
	export function MakeFromActiveFilters(
		...
		const wijmoActiveFilters: WijmoActiveFilters = JSON.parse(serializedActiveFilters);
		const activeFilters = new Array<OSFramework.DataGrid.OSStructure.ActiveFilter>();
		...
		wijmoActiveFilters.filters
			...
			.forEach((filter) => {
				const activeFilter = new OSFramework.DataGrid.OSStructure.ActiveFilter();
				...
				switch (activeFilter.filterTypeId) {
					case 'condition':
						...
						if (filter.condition1.operator !== null) {
							activeFilter.filterConditions.push(_createCondition(filter.condition1, filter.and));
						}
						if (filter.condition2.operator !== null) {
							activeFilter.filterConditions.push(_createCondition(filter.condition2, filter.and));
						}
						break;
					case 'value':
