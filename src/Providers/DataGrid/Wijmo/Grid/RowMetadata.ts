...
namespace Providers.DataGrid.Wijmo.Grid {
	export class RowMetadata implements OSFramework.DataGrid.Interface.IRowMetadata {
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		private _configs: any;
		private readonly _extraData = OSFramework.DataGrid.Enum.RowMetadata.Key;
		...
		constructor(grid: wijmo.grid.FlexGrid, configs: any) {
			this._grid = grid;
			this._itemsSource = this._grid.itemsSource as wijmo.collections.CollectionView;
		...
		private _getRowMetadataByRowKey(rowKey: string): Map<string, any> {
			...
			if (!this._hasMetadataByRowKey(rowIndex))
				this._itemsSource.sourceCollection[rowIndex][this._extraData] = new Map<
					string,
					// eslint-disable-next-line @typescript-eslint/no-explicit-any
					any
				>();

			return this._itemsSource.sourceCollection[rowIndex][this._extraData];
		}
		...
		private _hasMetadataByRowKey(row: number): boolean {
			return this._itemsSource.sourceCollection[row] && this._itemsSource.sourceCollection[row][this._extraData];
		}

		private _hasMetadataByRowNumber(row: number): boolean {
			return this._grid.rows[row]?.dataItem && this._grid.rows[row].dataItem[this._extraData];
		}
		...
		public clear(dataItem?: any): void {
			if (dataItem) {
				...
				this._itemsSource.sourceCollection.forEach((p) => delete p[this._extraData]);
			}
		}
		...
		public clearProperty(propertyName: string): void {
			...
			this._itemsSource.sourceCollection.forEach((p) => {
				p[this._extraData] &&
					// eslint-disable-next-line @typescript-eslint/no-explicit-any
					(p[this._extraData] as Map<string, any>).delete(propertyName);
			});
		}
		...
		public clearPropertyByRow(dataItem: any, propertyName: string): void {
			...
			if (dataItem && dataItem[this._extraData] && dataItem[this._extraData].has(propertyName)) {
		...
		public clearPropertyByRowKey(key: string, propertyName: string): void {
			this.hasOwnPropertyByRowKey(key, propertyName) && this._getRowMetadataByRowKey(key).delete(propertyName);
		}
		/**
		 ...
		 * @memberof RowMetadata
		 */
		public clearPropertyByRowNumber(rowNumber: number, propertyName: string): void {
		...
		public hasOwnPropertyByRow(row: any, property: string): boolean {
			return this._hasMetadataByRow(row) && this._getRowMetadataInRow(row).has(property);
		}

		public hasOwnPropertyByRowKey(rowKey: string, property: string): boolean {
			const rowIndex = this.getRowIndexByKey(rowKey);

			return this._hasMetadataByRowKey(rowIndex) && this._getRowMetadataByRowKey(rowKey).has(property);
		}

		public hasOwnPropertyByRowNumber(rowNumber: number, property: string): boolean {
			return this._hasMetadataByRowNumber(rowNumber) && this._getRowMetadataByRowNumber(rowNumber).has(property);
		}
		...
		public setMetadataByRowKey(
			...
			this._getRowMetadataByRowKey(rowKey).set(propertyName, propertyValue);
		}
		...
		public setMetadataByRowNumber(
			...
			this._getRowMetadataByRowNumber(rowNumber).set(propertyName, propertyValue);
		}
	}
}
