...
namespace Providers.DataGrid.Wijmo.Grid {
	export class ProviderDataSource extends OSFramework.DataGrid.Grid.AbstractDataSource {
		...
		private _getDirtyEditedItems() {
			...
			return itemsSource.itemsEdited.filter((editedItem) => {
				const rowIndex = itemsSource.sourceCollection.findIndex((item) => item === editedItem);
		...
		public getChanges<T extends OSFramework.DataGrid.OSStructure.ChangesDone>(c: new () => T): T {
			const changes = new c();
			const itemsSource = this._provider;

			if (itemsSource.itemsAdded.length > 0) {
				changes.hasChanges = true;
				changes.addedLinesJSON = this._getChangesString(itemsSource.itemsAdded);
			}

			if (itemsSource.itemsEdited.length > 0) {
				changes.hasChanges = true;
				changes.editedLinesJSON = this._getChangesString(itemsSource.itemsEdited);
			}

			if (itemsSource.itemsRemoved.length > 0) {
				changes.hasChanges = true;
				changes.removedLinesJSON = this._getChangesString(itemsSource.itemsRemoved);
			}
