...
namespace Providers.DataGrid.Wijmo.Grid {
		...
		implements IGridWijmo
	{
		...
		private _safari14workaround(): void {
			if (/^((?!chrome|android).).*Version\/14.*safari/i.test(navigator.userAgent)) {
				...
				OSFramework.DataGrid.Helper.Logger.LogInfo(
					'The fix for Safari 14 has been applied.',
					`Grid:${this.uniqueId}`
				);
			}
		}
		...
		public build(): void {
			...
			this.finishBuild();
			OSFramework.DataGrid.Helper.Logger.LogInfo('Grid built and ready', `Grid:${this.uniqueId}`);
		}
		...
		public changeColumnProperty(
			...
			if (!column) {
				...
				OSFramework.DataGrid.Helper.Logger.LogInfo(
					`changeColumnProperty - '${propertyName}' on column '${columnID}'`,
					`Grid:${this.uniqueId}`
				);
		...
		public dispose(): void {
			...
			OSFramework.DataGrid.Helper.Logger.LogInfo('Disposing provider grid', `Grid:${this.uniqueId}`);
		...
		public getColumnByProvider(
			...
			// `name` holds the OSFramework uniqueId, but autogenerated columns have no `name`; fall
			// back to `binding`, which still resolves their OSFramework column via equalsToID.
			const columnKey = providerColumn.name || providerColumn.binding;
