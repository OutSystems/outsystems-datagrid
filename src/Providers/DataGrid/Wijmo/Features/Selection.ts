...
namespace Providers.DataGrid.Wijmo.Feature {
	 ...
	 */
	export interface IProviderSelection extends OSFramework.DataGrid.Feature.ISelection {
		getProviderAllSelections(): wijmo.grid.CellRange[];
	}

	export class Selection implements IProviderSelection, OSFramework.DataGrid.Interface.IBuilder {
		...
		private _getCheckedRows(): number[] {
			return this._grid.provider.rows.filter((p) => p.isSelected).map((p) => p.index);
		}
		 ...
		 */
		private _selectionChanged(grid: wijmo.grid.FlexGrid, e: wijmo.grid.CellRangeEventArgs) {
			if (e.row >= 0) {
		...
		public build(): void {
			...
			this._grid.provider.selectionChanged.addHandler(this._selectionChanged.bind(this));

			if (this._grid.features.rowHeader.hasCheckbox) {
				this._grid.provider.updatingView.addHandler(this._updatingView.bind(this));
			}
		...
		public equalizeSelection(): OSFramework.DataGrid.OSStructure.CellRange[] {
			...
			if (this._grid.provider.selectionMode !== wijmo.grid.SelectionMode.MultiRange || this.hasCheckedRows())
				return;
			...
			grid.deferUpdate(() => {
				...
				[activeSelection, ...selectedRanges.slice()]
					...
					.forEach((curr, index, array) => {
						...
						if (rangesToRemove.filter((p) => p === curr).length > 0) return;
			...
			return grid.selectedRanges
				...
				.map((p) => Helper.CellRangeFactory.MakeFromProviderCellRange(p));
		}
		...
		public getActiveCell(): OSFramework.DataGrid.OSStructure.CellRange {
			...
			if (currSelection && currSelection.isValid)
				...
				return Helper.CellRangeFactory.MakeFromCoordinates(currSelection.row, currSelection.col);
		...
		public getAllSelectionsData(): Array<OSFramework.DataGrid.Types.RowData> {
			const rowColumn = new Map<number, OSFramework.DataGrid.OSStructure.RowData>();
							...
							...bindings.map(
								(binding) =>
									new OSFramework.DataGrid.OSStructure.BindingValue(
										binding,
										this._grid.provider.getCellData(rowIndex, binding, false)
									)
							)
						);
					});
		...
		public getCheckedRowsData(): Array<OSFramework.DataGrid.OSStructure.BindingValue> {
			const allCheckedRows = this._grid.provider.itemsSource.sourceCollection.filter(
				(item) => item?.__osRowMetadata?.get(this._internalLabel)?.isChecked === true
			);

			const allCheckedRowsArr = allCheckedRows.map(
				(dataItem) => new OSFramework.DataGrid.OSStructure.CheckedRowData(this._grid, dataItem)
			);

			return allCheckedRowsArr.map((p) => p.serialize());
		}

		public getMetadata(rowNumber: number): OSFramework.DataGrid.Feature.Auxiliar.RowSelection {
		...
		public getProviderAllSelections(): wijmo.grid.CellRange[] {
			...
			selectedRows.forEach((rowIndex) => {
				const rowRange = new wijmo.grid.CellRange(rowIndex, 0, rowIndex, maxCol);
					...
					selectedRowsRanges.forEach((rowRange) => {
						...
						if (range !== null && range.intersects(rowRange)) {
							...
							if (row1 < rowRange.row) {
								finalRange.push(
									new wijmo.grid.CellRange(row1, range.col, rowRange.row - 1, range.col2)
								);
							}
							...
							if (row2 > rowRange.row) {
								range = new wijmo.grid.CellRange(rowRange.row + 1, range.col, row2, range.col2);
							}
		...
		public getSelectedRowsCountByCellRange(): number {
			...
			return this.getAllSelections().reduce((acc, sel) => acc + (sel.bottomRowIndex - sel.topRowIndex + 1), 0);
		}
			...
			for (const item of _items) {
				item.selected.forEach((element) => {
					...
					if (
						...
						columnType === OSFramework.DataGrid.Enum.ColumnType.Currency ||
						columnType === OSFramework.DataGrid.Enum.ColumnType.Calculated
					) {
		...
		public setRowAsSelected(rowsIndex: number[], isSelected = true): number[] {
			...
			if (rowsIndex.find((index) => index >= rowsPerPage) !== undefined) {
				throw new Error(OSFramework.DataGrid.Enum.ErrorMessages.SetRowAsSelected);
			}
