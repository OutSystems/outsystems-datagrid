...
namespace Providers.DataGrid.Wijmo.Feature {
	export class GridInsertRowAction extends wijmo.undo.UndoableAction {
		...
		constructor(
			...
			OSFramework.DataGrid.Helper.BatchArray(undoableItems.items, (chunk) =>
		...
		public applyState(state: { action?: string; datasourceIdx?: number; items?: unknown[] }): void {
			...
			if (collectionView) {
				if (state.action === 'remove') {
					//undo
					collectionView.sourceCollection.splice(state.datasourceIdx, state.items.length);
					...
					collectionView.trackChanges && collectionView.itemsAdded.push(...state.items);
				}
			...
			if (collectionView) {
				if (state.action === 'insert') {
					...
					state.items
						...
						.forEach((item) => {
							this._removeItemFromCollectionView(collectionView, item);
						});
				} else {
					//redo
					state.forEach((item) => {
				...
				collectionView.refresh();
				collectionView.moveCurrentToPosition(collectionView.currentPosition);
				this._target.focus();
	...
	export class Rows implements OSFramework.DataGrid.Interface.IBuilder, OSFramework.DataGrid.Feature.IRows {
		...
		private _formatItems(grid: wijmo.grid.FlexGrid, e: wijmo.grid.FormatItemEventArgs) {
			if (e.panel.cellType === wijmo.grid.CellType.Cell && this.getMetadata(e.row).cssClass.size > 0) {
				for (const cssClass of this.getMetadata(e.row).cssClass.keys()) {
		...
		private _getTopRow(): number {
			const topRow = Math.min(
				...this._grid.features.selection.getAllSelections().map((cellRange) => cellRange.topRowIndex)
			);
		...
		private _validateAddNewRow(rowsAmount: number, topRowIndex: number): void {
			...
			if (rowsAmount < 1) {
				throw new Error(OSFramework.DataGrid.Enum.ErrorMessages.AddRowLowerThanOne);
			}

			// in client side validates if the amount of new rows exceeds in the current page
			if (
				this._grid.features.pagination.pageSize > 0 &&
				rowsAmount + topRowIndex > this._grid.features.pagination.pageSize
			) {
				throw new Error(OSFramework.DataGrid.Enum.ErrorMessages.AddRowExceedingPageSize);
			}

			// on serverSideGrids we don't have control of pageSize,
			// therefore we only want this validation on client side grids.
			if (this._grid.features.pagination.pageSize > 0 && rowsAmount > this._grid.features.pagination.pageSize) {
				throw new Error(OSFramework.DataGrid.Enum.ErrorMessages.AddRowGreaterThanPageSize);
			}
		}
		 ...
		 */
		public addClass(rowNumber: number, className: string, refresh: boolean, binding: string): void {
		...
		public addNewRows(rowsAmount: number): number[] {
			...
			let dsTopRowIndex = topRowIndex + this._grid.features.pagination.rowStart - 1;
			...
			const quantity =
				rowsAmount > 1
					...
					: // Consider the quantity 1 if there is no selection.
						this._grid.features.selection.getSelectedRowsCountByCellRange() || 1;
			...
			items = this._grid.provider.collectionView.sourceCollection.slice(dsTopRowIndex, dsTopRowIndex + quantity);
			const undoableItems = { datasourceIdx: dsTopRowIndex, items };
			this._grid.features.undoStack.pushAction(new GridInsertRowAction(this._grid, undoableItems));
			...
			if (this._getRowsCount() === expectedRowCount) {
				...
				throw new Error(OSFramework.DataGrid.Enum.ErrorMessages.AddRowErrorMessage);
			}
		 ...
		 */
		public getMetadata(rowNumber: number): OSFramework.DataGrid.Feature.Auxiliar.RowStyleInfo {
		 ...
		 */
		public removeClass(rowNumber: number, className: string, refresh = false, binding = ''): void {
			const row = this.getMetadata(rowNumber);
			if (!row) {
				throw new Error(OSFramework.DataGrid.Enum.ErrorMessages.Row_NotFound);
			}
		 ...
		 */
		public removeRowsByNumberOrKey(rowNumbersList: Array<number>, rowKeysList: Array<string>): void {
			...
			const rowsList = rowNumbersList.length > 0 ? rowNumbersList : rowKeysList;
			...
			dataSource.deferUpdate(() => {
				rowsList.forEach((row) => {
					const rowIndex = typeof row === 'number' ? row : this._grid.rowMetadata.getRowIndexByKey(row);

					if (rowIndex < 0 || !dataSource.sourceCollection[rowIndex]) {
					...
					this._grid.features.validationMark.setRowStatusByNumber(rowIndex, true);

					this._grid.features.undoStack.pushAction(new GridRemoveRowAction(this._grid, undoableItems));
				});
			...
			if (this._getRowsCount() === expectedRowCount) {
				...
				throw new Error(OSFramework.DataGrid.Enum.ErrorMessages.RemoveRowErrorMessage);
			}
		}
		...
		public removeSelectedRows(): void {
			...
			const expectedRowCount =
				this._getRowsCount() - this._grid.features.selection.getSelectedRowsCountByCellRange();
			const selRanges = (this._grid.features.selection as IProviderSelection).getProviderAllSelections();
			...
			dataSource.deferUpdate(() => {
				selRanges.forEach((range) => {
					for (let row = range.bottomRow; row >= range.topRow; row--) {
						const undoableItem = {
							datasourceIdx: row,
						...
						this._grid.features.validationMark.setRowStatus(row, true);
					}
					this._grid.features.undoStack.pushAction(new GridRemoveRowAction(this._grid, undoableItems));
				});
			});
