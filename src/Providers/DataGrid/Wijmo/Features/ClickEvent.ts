			...
			OSFramework.DataGrid.Interface.IDisposable
	{
		...
		private _raiseCellClickEvent(e: MouseEvent) {
			...
			if (ht.cellType === wijmo.grid.CellType.Cell) {
				...
				const uniqueId = column.name; // column.name holds the OS column identifier (uniqueId, or binding for metadata-autogenerated columns)
				const columnWidgetId = this._grid.getColumn(uniqueId)?.widgetId || binding; // if no column exists (Wijmo-only autogenerated columns using JSONSerialize), then use the binding
				const line = OSFramework.DataGrid.Helper.DeepClone(this._grid.provider.rows[rowNumber].dataItem);
