...
namespace Providers.DataGrid.Wijmo.Feature {
	...
	function FormatDate(comparedValue: any, cellValue: any) {
		...
		if (comparedYear <= 1911) {
			...
			comparedValue =
				`${comparedYear}-0${(comparedDate.getUTCMonth() + 1).toString().slice(-2)}-0${comparedDate
					.getUTCDate()
					.toString()
					.slice(-2)}T` + timezoneOffset;
		}

		const formattedCellValue = Helper.DataUtils.GetTicksFromDate(cellValue, comparedValue.indexOf('Z') > -1);
		const formatedComparedValue = Date.parse(Helper.DataUtils.ResetSeconds(comparedValue));
	...
	function Evaluate(operator: Rules, comparedValue: any, cellValue: any = 0) {
		...
		if (cellValue && typeof cellValue.getMonth === 'function') {
			const [formattedComparedValue, formattedCellValue] = FormatDate(comparedValue, cellValue);
	...
	class ConditionAnd {
		...
		constructor(cellClass: string, rowClass: string, rules: Array<Condition>) {
		...
		public evaluate(
			...
			const evaluated = this.rules.map((rule) => {
				if (
					columnType === OSFramework.DataGrid.Enum.ColumnType.Dropdown ||
	...
	class ConditionExecuter {
		...
		constructor(conditions: Array<ConditionAnd>, grid: Grid.IGridWijmo, isAggregate = false) {
		 ...
		 * @param shouldAdd
		 */
		private _addOrRemoveAggregateClass(columnBinding: string, className: string, shouldAdd: boolean) {
			if (className) {
				if (shouldAdd) {
					this._grid.features.columnAggregate.addClass(columnBinding, className);
				} else {
					this._grid.features.columnAggregate.removeClass(columnBinding, className);
				}
			}
		}
		 ...
		 * @param shouldAdd
		 */
		private _addOrRemoveCellClass(columnBinding: string, cellClass: string, rowNumber: number, shouldAdd: boolean) {
			if (cellClass) {
				if (shouldAdd) {
					this._grid.features.cellStyle.addClass(columnBinding, rowNumber, cellClass);
				} else {
					this._grid.features.cellStyle.removeClass(rowNumber, columnBinding, cellClass);
				}
			}
		}
		 ...
		 * @param shouldAdd
		 */
		private _addOrRemoveRowClass(columnBinding: string, rowClass: string, rowNumber: number, shouldAdd: boolean) {
			if (rowClass) {
				if (shouldAdd) {
					this._grid.features.rows.addClass(rowNumber, rowClass, false, columnBinding);
				} else {
					this._grid.features.rows.removeClass(rowNumber, rowClass, false, columnBinding);
				}
			}
		}
			...
			this.conditions
				...
				.forEach((condition) => {
					...
					if (this._isAggregate) {
						this._addOrRemoveAggregateClass(binding, condition.cellClass, condition.isTrue);
					} else {
						this._addOrRemoveRowClass(binding, condition.rowClass, e.row, condition.isTrue);

						this._addOrRemoveCellClass(binding, condition.cellClass, e.row, condition.isTrue);
					}
				});
		}
	}
	...
	export class ConditionalFormat
		implements OSFramework.DataGrid.Feature.IConditionalFormat, OSFramework.DataGrid.Interface.IBuilder
	{
		private _grid: Grid.IGridWijmo;
		...
		private _parseRule(
			...
			rules.forEach((element) => {
				...
				const conditionAnds = new ConditionAnd(element.cellClass, element.rowClass, conditions);
				conditionExecuters.push(conditionAnds);
			});
			return new ConditionExecuter(conditionExecuters, this._grid, isAggregate);
		}
		...
		private _updateAggregateRows() {
			...
			const columnsAggregate = this._grid
				.getColumns()
				.filter((column) => this._mappedRulesAggregate.get(column.config.binding));
			...
			columnsAggregate.forEach((column) => {
				...
				// OSFramework uniqueId), falling back to `binding` for autogenerated columns (no `name`).
				// The resolved `.index` is read fresh.
				const providerColumn = this._grid.provider.columns.find(
					(wijmoColumn) =>
						wijmoColumn.name === column.uniqueId ||
						(!wijmoColumn.name && wijmoColumn.binding === column.provider.binding)
				);
		...
		private _updateRows() {
			const columns = this._grid.getColumns().filter((column) => this._mappedRules.get(column.config.binding));
		...
		public addAggregateRules(
			...
			this._mappedRulesAggregate.set(binding, this._parseRule(rules, true));
		}
		...
		public build(): void {
			this._grid.provider.updatingView.addHandler(this._updatingViewHandler.bind(this));
		}
