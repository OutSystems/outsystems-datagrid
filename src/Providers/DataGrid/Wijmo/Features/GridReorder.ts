...
namespace Providers.DataGrid.Wijmo.Feature {
	export class GridReorder
		implements OSFramework.DataGrid.Feature.IGridReorder, OSFramework.DataGrid.Interface.IBuilder
	{
		...
		private _addColumnDragEvents(): void {
			this._grid.provider.draggingColumn.addHandler(this._draggingColumnHandler.bind(this));
			this._grid.provider.draggingColumnOver.addHandler(this._draggingColumnOverHandler.bind(this));
			this._grid.provider.draggedColumn.addHandler(this._draggedColumnHandler.bind(this));
		}

		private _addRowDragEvents(): void {
			this._grid.provider.draggingRow.addHandler(this._draggingRowHandler.bind(this));
			this._grid.provider.draggedRow.addHandler(this._draggedRowHandler.bind(this));
		}

		private _draggedColumnHandler(_s: wijmo.grid.FlexGrid, e: wijmo.grid.CellRangeEventArgs) {
		...
		private _draggedRowHandler(s: wijmo.grid.FlexGrid, e: wijmo.grid.CellRangeEventArgs): void {
		...
		private _draggingRowHandler(s: wijmo.grid.FlexGrid, e: wijmo.grid.CellRangeEventArgs): void {
			if (this._grid.features.sort.isGridSorted) {
				throw new Error(OSFramework.DataGrid.Enum.ErrorMessages.ReorderRowWithActiveSort);
			}
		...
		private _removeRowDragEvents(): void {
			this._grid.provider.draggingRow.removeHandler(this._draggingRowHandler);
			this._grid.provider.draggedRow.removeHandler(this._draggedRowHandler);
		}
		...
		private _setAllowDragging() {
			...
			if (this._allowColumnDragging && this._allowRowDragging) {
				this._grid.provider.allowDragging = wijmo.grid.AllowDragging.Both;
			} else if (!this._allowColumnDragging && this._allowRowDragging) {
				this._grid.provider.allowDragging = wijmo.grid.AllowDragging.Rows;
			} else if (this._allowColumnDragging && !this._allowRowDragging) {
				this._grid.provider.allowDragging = wijmo.grid.AllowDragging.Columns;
			}
		}
		...
		public toggleRowDragging(allowRowDragging: boolean): void {
			if (allowRowDragging) {
				...
				if (this._grid.features.rowHeader.hasCheckbox) {
					throw new Error(OSFramework.DataGrid.Enum.ErrorMessages.ReorderRowOnGridWithCheckbox);
				}
			} else {
