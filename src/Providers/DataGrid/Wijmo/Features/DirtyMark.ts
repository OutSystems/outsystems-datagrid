		...
		private _isDirtyCell(row: number, col: number): boolean {
			...
			if (this.hasMetadata(row)) {
				const columnUniqueId =
					Helper.GetColumnUniqueId(this._grid.provider.columns[col]) ||
					this._grid.provider.columns[col].binding; // if the uniqueId is undefined (in the case of autogenerated columns), then use the binding
