		...
		private _isDirtyCell(row: number, col: number): boolean {
			...
			if (this.hasMetadata(row)) {
				const columnUniqueId =
					this._grid.provider.columns[col].name || this._grid.provider.columns[col].binding; // column.name holds the OS uniqueId; if undefined (autogenerated columns), use the binding
