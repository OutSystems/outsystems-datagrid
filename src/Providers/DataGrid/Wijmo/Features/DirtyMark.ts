...
namespace Providers.DataGrid.Wijmo.Feature {
	export class DirtyMark implements OSFramework.DataGrid.Feature.IDirtyMark, OSFramework.DataGrid.Interface.IBuilder {
		private _grid: Grid.IGridWijmo;
		private readonly _internalLabel = OSFramework.DataGrid.Enum.RowMetadata.DirtyMark;
		...
		private _formatItems(_grid: wijmo.grid.FlexGrid, e: wijmo.grid.FormatItemEventArgs) {
			if (e.panel.cellType === wijmo.grid.CellType.Cell && this._isDirtyCell(e.row, e.col)) {
				wijmo.addClass(e.cell, 'dirty-mark');
			} else if (e.panel.cellType === wijmo.grid.CellType.RowHeader && this._isDirtyRow(e.row)) {
				wijmo.addClass(e.cell, 'dirty-mark');
			}
		}

		private _hasCellInitialValue(row: number, binding: string): boolean {
			return this.hasMetadata(row) && this.getMetadata(row).originalValues.has(binding);
		}

		private _isDirtyCell(row: number, col: number): boolean {
			const grid = this._grid.provider;
			if (this.hasMetadata(row)) {
				const providerColumn = this._grid.provider.columns[col];
				const columnUniqueId = providerColumn.name || providerColumn.binding; // column.name holds the OS uniqueId; if undefined (autogenerated columns), use the binding
				const targetColumn = this._grid.getColumn(columnUniqueId);
				// targetColumn can be undefined when the column is not in the OS column map (e.g.
				// autogenerated columns); fall back to the provider column binding, consistent with
				// the `targetColumn &&` guard below and with ClickEvent's optional chaining.
				const binding = targetColumn?.config.binding || providerColumn.binding;
				let isActionColumn = false;

				//If the grid is autogenerated, no need to check for action columns.
		...
		private _isDirtyRow(row: number): boolean {
			if (this.hasMetadata(row)) {
				...
				const hasDirtyMarkCell = keyArray.some((k) => {
					...
					const cellValue = this._grid.provider.getCellData(row, k, false);
		...
		public get isGridDirty(): boolean {
			return this._grid.provider.itemsSource.sourceCollection.some((_row, index) => this._isDirtyRow(index));
		}
		...
		public build(): void {
			...
			this._grid.provider.beginningEdit.addHandler(this._cellEditHandler.bind(this));
			this._grid.provider.pastingCell.addHandler(this._cellEditHandler.bind(this));
			this._grid.addedRows.addHandler(this._addNewRowEndingHandler.bind(this));
		...
		public getMetadata(row: number): OSFramework.DataGrid.Feature.Auxiliar.DirtyMarksInfo {
			if (!this.hasMetadata(row))
