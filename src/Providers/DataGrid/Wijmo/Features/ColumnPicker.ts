...
namespace Providers.DataGrid.Wijmo.Feature {
	...
	class ColumnPickerAction extends wijmo.undo.UndoableAction {
		...
		constructor(grid: Grid.IGridWijmo, listBox: wijmo.input.ListBox, binding: string, state: boolean) {
			super(grid.provider);
			this._grid = grid;
		...
		public applyState(state: boolean): void {
			...
			this._col.provider.visible = state !== undefined ? state : !this._oldState;
			this._listBox.loadList();
			this._grid.features.columnPicker.handleColumnPickerChangeEvent(this._col.provider);
			this.target.focus();
		}
			...
			OSFramework.DataGrid.Interface.IDisposable
	{
		...
		private _filterAutogenColumns(column) {
			return (
				this._grid.provider.itemsSource.groupDescriptions.filter((q) => q.propertyName === column.binding)
					.length === 0
			);
		}
		...
		private _getColumnsToBeDisplayedOnColumnPicker(): any[] {
			...
			if (isAutogenerated) {
				return this._grid.provider.columns.filter((col) => this._filterAutogenColumns(col));
			}

			return (
				columns
					.filter((col) => this._filterColumnsToBeDisplayedOnColumnPicker(col))
		...
		private _makeColumnPicker(): void {
			...
			theGrid.formatItem.addHandler((s: wijmo.grid.FlexGrid, e: wijmo.grid.FormatItemEventArgs) => {
			...
			this._theColumnPicker = new wijmo.input.ListBox(picker, {
				...
				itemChecked: (s: wijmo.input.ListBox) => {
					...
					this._grid.features.undoStack.startAction(
						new ColumnPickerAction(this._grid, this._theColumnPicker, _item.binding, !_item.visible)
					);
					this._grid.features.undoStack.closeAction(ColumnPickerAction, false);
				},
				lostFocus: () => {
			...
			span.onclick = (e: MouseEvent) => {
				if (!host.offsetHeight) {
					this._theColumnPicker.itemsSource = this._getColumnsToBeDisplayedOnColumnPicker();
		...
		public handleColumnPickerChangeEvent(column: wijmo.grid.Column): void {
			if (this._grid.gridEvents.hasHandlers(OSFramework.DataGrid.Event.Grid.GridEventType.OnColumnPickerChange)) {
				const _column = this._grid.getColumn(column.binding);
				this._grid.gridEvents.trigger(
					OSFramework.DataGrid.Event.Grid.GridEventType.OnColumnPickerChange,
					this._grid,
					_column.widgetId,
					column.binding,
					column.isVisible
