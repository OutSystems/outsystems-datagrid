...
namespace Providers.DataGrid.Wijmo.Feature {
	export class RowHeader implements OSFramework.DataGrid.Feature.IRowHeader, OSFramework.DataGrid.Interface.IBuilder {
		...
		public get hasRowNumber(): boolean {
			return (
				this._rowHeaderType === OSFramework.DataGrid.Enum.RowHeader.RowNumber ||
				this._rowHeaderType === OSFramework.DataGrid.Enum.RowHeader.RowNumberAndCheckbox
			);
		}

		public get hasCheckbox(): boolean {
			return (
				this._rowHeaderType === OSFramework.DataGrid.Enum.RowHeader.RowCheckbox ||
				this._rowHeaderType === OSFramework.DataGrid.Enum.RowHeader.RowNumberAndCheckbox
			);
		}
		...
		private _buildRowHeader() {
			if (this.hasCheckbox) {
				...
				this._grid.provider.selectionMode = wijmo.grid.SelectionMode.MultiRange;
				...
				this._grid.provider.formatItem.addHandler((s, e) => {
					if (e.panel.cellType === wijmo.grid.CellType.RowHeader) {
						const columnHeaderElement = s.columnHeaders.hostElement.querySelector('.wj-cell.wj-header');

						const rowHeaderCheckedElement = s.rowHeaders.hostElement.querySelector(
							"input[type='checkbox']:checked"
						);

						if (rowHeaderCheckedElement) {
							wijmo.addClass(columnHeaderElement, 'checked-column-header');
						} else if (wijmo.hasClass(columnHeaderElement, 'checked-column-header')) {
							wijmo.removeClass(columnHeaderElement, 'checked-column-header');
						}
					}
				});
			}
