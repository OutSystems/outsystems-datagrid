			...
			OSFramework.DataGrid.Interface.IDisposable
	{
		...
		private _filterChangedHandler(s: wijmo.grid.filter.FlexGridFilter) {
			...
			if (this._grid.gridEvents.hasHandlers(OSFramework.DataGrid.Event.Grid.GridEventType.OnFiltersChange)) {
				this._grid.gridEvents.trigger(
					OSFramework.DataGrid.Event.Grid.GridEventType.OnFiltersChange,
					this._grid,
					Helper.FilterFactory.MakeFromActiveFilters(this._grid, s.filterDefinition)
				);
			}
		}

		private _filterChangingHandler(s: wijmo.grid.filter.FlexGridFilter) {
			this._grid.features.undoStack.startAction(new ColumnFilterAction(s));
		}
		...
		public get isGridFiltered(): boolean {
			...
			return (
				...
				//@ts-ignore
				this._filter._filters.filter((columnFilter) => columnFilter.isActive).length > 0
			);
		}
		...
		private _isValidDate(value: string): boolean {
			...
			return value && !value.startsWith(OSFramework.DataGrid.Constants.OSNullDate);
		}
		 ...
		 */
		public build(): void {
			this._filter = new wijmo.grid.filter.FlexGridFilter(this._grid.provider);
			this._filter.filterChanging.addHandler(this._filterChangingHandler.bind(this));

			this._filter.filterChanged.addHandler(this._filterChangedHandler.bind(this));
		...
		public byCondition(columnId: string, values: OSFramework.DataGrid.OSStructure.FilterCondition[]): void {
			...
			if (column) {
				...
				const isNumericalColumn =
					column.columnType === OSFramework.DataGrid.Enum.ColumnType.Calculated ||
				...
				if (values.length > 0) {
					...
					if (
						isNumericalColumn &&
						(isNaN(parseFloat(condition1.value)) || (condition2 && isNaN(parseFloat(condition2.value))))
					) {
						throw new Error(OSFramework.DataGrid.Enum.ErrorMessages.Filter_InvalidDataType);
					}

					columnFilter.condition1.value = this._getFilterConditionValue(
						column.columnType,
						isNumericalColumn ? parseFloat(condition1.value) : condition1.value
					);
					columnFilter.condition1.operator = wijmo.grid.filter.Operator[condition1.operatorTypeId];
					columnFilter.and = condition1.and;

					if (condition2) {
						columnFilter.condition2.value = this._getFilterConditionValue(
							column.columnType,
							isNumericalColumn ? parseFloat(condition2.value) : condition2.value
						);
						columnFilter.condition2.operator = wijmo.grid.filter.Operator[condition2.operatorTypeId];
					}
		...
		public byValue(columnId: string, values: Array<string>): void {
			...
			if (column) {
				...
				const isColumnTypeDate =
					column.columnType === OSFramework.DataGrid.Enum.ColumnType.Date ||
					column.columnType === OSFramework.DataGrid.Enum.ColumnType.DateTime;

				const columnFilter = this._filter.getColumnFilter(column.provider).valueFilter;
		 ...
		 */
		public changeFilterType(columnID: string, filterType: wijmo.grid.filter.FilterType): void {
			const column = this._grid.getColumn(columnID);

			if (column) {
				this._filter.getColumnFilter(column.provider).filterType = filterType;
			} else {
				throw new Error(OSFramework.DataGrid.Enum.ErrorMessages.InvalidColumnIdentifier);
			}
		}
		 ...
		 */
		public setColumnFilterOptions(columnID: string, options: Array<string>, maxVisibleOptions?: number): void {
			...
			if (column) {
				...
				if (
					column.columnType === OSFramework.DataGrid.Enum.ColumnType.Text ||
					column.columnType === OSFramework.DataGrid.Enum.ColumnType.Dropdown ||
					column.columnType === OSFramework.DataGrid.Enum.ColumnType.Checkbox ||
					column.columnType === OSFramework.DataGrid.Enum.ColumnType.Currency ||
					column.columnType === OSFramework.DataGrid.Enum.ColumnType.Date ||
					column.columnType === OSFramework.DataGrid.Enum.ColumnType.DateTime ||
					column.columnType === OSFramework.DataGrid.Enum.ColumnType.Number
					...
					this.changeFilterType(columnID, wijmo.grid.filter.FilterType.Both);

					this._filter.getColumnFilter(column.provider).valueFilter.uniqueValues = Array.from(
						new Set<string>(options) // we only want unique values
					);

					if (maxVisibleOptions > 0)
						this._filter.getColumnFilter(column.provider).valueFilter.maxValues = maxVisibleOptions;
				} else {
					throw new Error(
		...
		public setState(value: boolean): void {
			this._filter.defaultFilterType = value ? this.filterType : wijmo.grid.filter.FilterType.None;
