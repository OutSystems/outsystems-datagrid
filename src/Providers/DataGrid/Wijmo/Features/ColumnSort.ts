...
namespace Providers.DataGrid.Wijmo.Feature {
	...
	export class ColumnSort
		implements OSFramework.DataGrid.Feature.IColumnSort, OSFramework.DataGrid.Interface.IBuilder
	{
		...
		private _makeActiveSort(
			...
			const activeSorts = new Array<OSFramework.DataGrid.OSStructure.ActiveSort>();

			if (sortDescriptions && sortDescriptions.length) {
				sortDescriptions.map((sd) => {
					const sort = new OSFramework.DataGrid.OSStructure.ActiveSort();
		...
		private _sortedHandler(
			...
			if (this._grid.gridEvents.hasHandlers(OSFramework.DataGrid.Event.Grid.GridEventType.OnSortChange)) {
		 ...
		 * @memberof ColumnSort
		 */
		private _sortingHandler(s: wijmo.grid.FlexGrid, e: wijmo.grid.CellRangeEventArgs) {
			const col = s.getColumn(e.col);
			const index = col.currentSortIndex;
			const sd = s.itemsSource.sortDescriptions[index];
			...
			if (this._hasUnsortState) {
				s.itemsSource.deferUpdate(() => {
					...
					if (sd && !sd.ascending) {
						...
						this._grid.features.undoStack.startAction(new ColumnSortAction(this._grid.provider));
		...
		public build(): void {
			this._grid.provider.sortingColumn.addHandler(this._sortingHandler.bind(this));
			this._grid.provider.sortedColumn.addHandler(this._sortedHandler.bind(this));
		...
		public getViewLayout(): any {
			return this._grid.provider.itemsSource.sortDescriptions.map((sortDesc) => {
				return {
		 ...
		 * @memberof ColumnSort
		 */
		public sortColumn(columnID: string, sorting: OSFramework.DataGrid.OSStructure.Sorting): void {
			const column = this._grid.getColumn(columnID);
			const ascending =
			...
			if (column) {
				...
				const existingColumnSort = this._grid.provider.itemsSource.sortDescriptions.find(
					(sd) => sd.property === column.config.binding
				);
				// if there is sort active on column, we must remove it
				if (existingColumnSort) {
					this._grid.provider.itemsSource.sortDescriptions.remove(existingColumnSort);
				}

				this._grid.provider.itemsSource.sortDescriptions.push(
					new wijmo.collections.SortDescription(column.config.binding, ascending)
				);
			} else {
