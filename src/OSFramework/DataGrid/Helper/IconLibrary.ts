...
namespace OSFramework.DataGrid.Helper {
	 ...
	 * Normalizes iconLibrary (strips digits/dots) and adds class `iconLibrary-${normalized}`.
	 *
	 * @export
	...
	export function ApplyIconLibraryClass(iconLibrary: string): void {
		...
		if (htmlElement) {
			if (currentIconLibrary) {
				htmlElement.classList.remove(`iconLibrary-${currentIconLibrary}`);
			}
			htmlElement.classList.add(`iconLibrary-${newIconLibrary}`);
		}

		SetIconLibrary(newIconLibrary);
	}
}