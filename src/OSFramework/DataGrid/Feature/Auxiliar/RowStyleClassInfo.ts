...
namespace OSFramework.DataGrid.Feature.Auxiliar {
	export class RowStyleInfo {
		...
		public addClass(className: string, binding: string): void {
			this._handleClassNames(className, binding, this._addClassName.bind(this));
		}

		/** Remove all classes from the cssClass array */
		public removeAllClasses(): void {
			Array.from(this.cssClass.keys()).forEach((key) => this.cssClass.delete(key));
		}

		/** Remove a single class from the cssClass array */
		public removeClass(className: string, binding: string): void {
			this._handleClassNames(className, binding, this._removeClassName.bind(this));
		}
	}
}
