...
namespace OSFramework.DataGrid.Event.Grid {
	 ...
	 */
	export class GridEventsManager extends AbstractEventsManager<GridEventType, OSFramework.DataGrid.Grid.IGrid> {
		...
		public addHandler(eventType: GridEventType, handler: OSFramework.DataGrid.Callbacks.OSGrid.Event): void {
			...
			if (eventType === GridEventType.Initialized && this._grid.isReady) {
				...
				Helper.AsyncInvocation(handler, this._grid.widgetId, this._grid);
			} else {
		...
		public trigger(
			...
			if (this.events.has(event)) {
				this.events.get(event).trigger(gridObj, gridObj.widgetId, ...args);
			}
		}
	}
}
