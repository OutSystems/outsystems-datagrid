		...
		implements IConfigurationGrid
	{
		...
		public getProviderConfig(): DataGrid.Types.IGridProviderConfigs {
			...
			let provider: DataGrid.Types.IGridProviderConfigs = {
				...
				// Wijmo 5.20261.52 (WJM-37524) handles Ctrl+X while autoClipboard is on. The grid has no cut
				// hook, so cut-cleared cells would bypass validation marks, OnCellValueChange and undo/redo.
				// Opt out to preserve the pre-upgrade contract; the key event still reaches the browser.
				preventCut: true,
