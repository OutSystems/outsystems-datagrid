...
namespace OSFramework.DataGrid.Configuration.Column {
	...
	export class EditorConfigNumber extends AbstractEditorConfig {
		...
		constructor(config: DataGrid.Types.INumberColumnExtraConfigs | DataGrid.Types.ICurrencyColumnExtraConfigs) {
			super(config);
		...
		public getProviderConfig(): DataGrid.Types.INumberEditorProviderConfigs {
			...
			let provider = {
				...
				min:
					...
					this.minValue !== undefined && this.minValue > this.minPerDecPlaces
						? this.minValue
				...
				max:
					...
					this.maxValue !== undefined && this.maxValue < this.maxPerDecPlaces
						? this.maxValue
