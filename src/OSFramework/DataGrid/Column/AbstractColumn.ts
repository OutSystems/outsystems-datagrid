...
namespace OSFramework.DataGrid.Column {
	 ...
	 * @abstract
	 */
	export abstract class AbstractColumn<T extends Configuration.IConfigurationColumn> implements IColumn {
		...
		protected _preBuild(): void {
			...
			if (!this._configs.autoGenerated) {
				...
				let widget = unique.closest(Helper.Constants.outsystemsWidgetTag);

				//If available save its Id
				if (this.config.genericColumnId === widget.id) {
					//Look find the parent data-block of the genericCol
					widget = widget.parentElement.closest(Helper.Constants.outsystemsWidgetTag);
				}

				this._widgetId = widget.id;
				this._parentGroupId = this._findParentGroup(widget.parentElement);
			}

			this._preBuilt = true;
		}

		protected setConditionalFormat(conditionalFormat: Array<OSStructure.ConditionalFormat>, refresh = false): void {
			if (conditionalFormat && conditionalFormat.length > 0) {
				this.grid.features.conditionalFormat.addRules(this.config.binding, conditionalFormat, refresh);
			}
		}
		...
		public changeProperty(propertyName: string, propertyValue: any): void {
			...
			if (this.config.hasOwnProperty(propertyName)) {
				...
				if (this.hasConditionalFormat && propertyName === OSStructure.ColumnProperties.ConditionalFormat) {
		...
		public get hasConditionalFormat(): boolean {
			return this.config.conditionalFormat !== undefined && this.config.conditionalFormat.length > 0;
		}
