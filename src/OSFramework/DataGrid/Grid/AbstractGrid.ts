...
namespace OSFramework.DataGrid.Grid {
	export abstract class AbstractGrid<W, Z extends Configuration.IConfigurationGrid> implements IGridGeneric<W> {
			...
			if (this.dataSource.hasMetadata) {
				...
				if (this.config.allowAutoGeneratedGrid) {
					...
					if (this.config.allowEdit === false) {
						...
						throw new Error(Enum.ErrorMessages.AutoGenerateGridWrite);
					}
				} else {
					//if the grid is not allowed to auto generate the columns (ODC), then this error will be displayed.
					throw new Error(Enum.ErrorMessages.AutoGenerateGrid);
				}
			}
		}
		...
		private _checkForNewColumns(): boolean {
			...
			const columns = Array.from(this._columns.values()).map((col) => col.config.binding);
		...
		private _validateBindings(): void {
			if (this.dataSource.hasMetadata) {
				...
				this.getColumns().forEach((column) => {
					...
					if (column.config.hasOwnProperty('parentBinding') && column.config['parentBinding'] !== '') {
		...
		public abstract clearAllChanges(forceClearValidationMarks: boolean): void;

		public abstract clearAllChangesByRowKeys(rowKeys: Array<string>, forceClearValidationMarks: boolean): void;
		...
		public abstract setCellError(binding: string, row: number, message: string): void;
