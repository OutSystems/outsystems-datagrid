namespace OutSystems.GridAPI.ColumnPicker {
	...
	export function SetColumnVisibility(
		...
		const result = Auxiliary.CreateApiResponse({
			...
			callback: () => {
				GridManager.GetGridById(gridID).features.columnPicker.setShowHiddenColumns(showHiddenColumns);
			},
		});
...
namespace GridAPI {
	...
	export namespace ColumnPicker {
		...
		export function SetColumnVisibility(
			...
			return OutSystems.GridAPI.ColumnPicker.SetColumnVisibility(gridID, showHiddenColumns);
		}
	}
}
