	...
	export function AddConditionalFormat(
		...
		GridManager.Events.Subscribe(
			...
			(_gridId: string, gridObj: OSFramework.DataGrid.Grid.IGrid) => {
				...
				Performance.SetMark('ConditionalFormat.AddConditionalFormat-end');
	...
	export function SetNumberAggregateConditionalFormatting(
		...
		const result = Auxiliary.CreateApiResponse({
			...
			callback: () => {
				...
				grid.features.columnAggregate.setConditionalFormat(columnID, conditionalFormat);
			},
		});

		Performance.SetMark('ColumnManager.SetNumberAggregateConditionalFormatting-end');
		Performance.GetMeasure(
	...
	export function RemoveConditionalFormat(gridID: string, binding: string): void {
		GridManager.Events.Subscribe(
			...
			(_gridId: string, gridObj: OSFramework.DataGrid.Grid.IGrid) => {
				...
				Performance.SetMark('ConditionalFormat.RemoveConditionalFormat-end');
				Performance.GetMeasure(
...
namespace GridAPI.ConditionalFormat {
	...
	export function AddConditionalFormat(
		...
		return OutSystems.GridAPI.ConditionalFormat.AddConditionalFormat(gridID, binding, rules);
	}
	...
	export function RemoveConditionalFormat(gridID: string, binding: string): void {
		...
		return OutSystems.GridAPI.ConditionalFormat.RemoveConditionalFormat(gridID, binding);
	}
}
