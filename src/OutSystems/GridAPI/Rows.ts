namespace OutSystems.GridAPI.Rows {
	...
	export function AddClass(gridID: string, rowNumber: number, className: string): string {
		...
		const result = Auxiliary.CreateApiResponse({
			...
			callback: () => {
				GridManager.GetGridById(gridID).features.rows.addClass(rowNumber, className, true);
			},
		});
	...
	export function AddRows(gridID: string, numberOfRows = 1): string {
		...
		const result = Auxiliary.CreateApiResponse({
			...
			callback: () => {
				return JSON.stringify(GridManager.GetGridById(gridID).features.rows.addNewRows(numberOfRows));
			},
			hasValue: true,
		});
	...
	export function GetRowData(gridID: string, rowNumber: number): string {
		...
		const result = Auxiliary.CreateApiResponse({
			...
			callback: () => {
				return JSON.stringify(GridManager.GetGridById(gridID).features.rows.getRowData(rowNumber));
			},
			hasValue: true,
		});
	...
	export function GetRowNumberByKey(gridID: string, key: string): string {
		...
		const result = Auxiliary.CreateApiResponse({
			...
			callback: () => {
				return JSON.stringify(GridManager.GetGridById(gridID).dataSource.getRowNumberByKey(key));
			},
			hasValue: true,
			defaultFailValue: -1,
		});
	...
	export function RemoveAllClasses(gridID: string, rowNumber: number): string {
		...
		const result = Auxiliary.CreateApiResponse({
			...
			callback: () => {
				GridManager.GetGridById(gridID).features.rows.removeAllClasses(rowNumber);
			},
		});
	...
	export function RemoveClass(gridID: string, rowNumber: number, className: string): string {
		...
		const result = Auxiliary.CreateApiResponse({
			...
			callback: () => {
				GridManager.GetGridById(gridID).features.rows.removeClass(rowNumber, className, true);
			},
		});
	...
	export function RemoveRowsByNumberOrKey(gridID: string, rowNumbers: string, rowKeys: string): string {
		...
		const result = Auxiliary.CreateApiResponse({
			...
			callback: () => {
				GridManager.GetGridById(gridID).features.rows.removeRowsByNumberOrKey(
	...
	export function RemoveRows(gridID: string): string {
		...
		const result = Auxiliary.CreateApiResponse({
			gridID,
			errorCode: OSFramework.DataGrid.Enum.ErrorCodes.API_FailedRemoveSelectedRow,
			callback: () => {
				GridManager.GetGridById(gridID).features.rows.removeSelectedRows();
			},
		});
	...
	export function UpdateAddedRowKey(gridID: string, currentRowId: string, newKey: string): string {
		...
		const result = Auxiliary.CreateApiResponse({
			gridID,
			errorCode: OSFramework.DataGrid.Enum.ErrorCodes.API_FailedUpdateAddedRowKey,
			callback: () => {
				return JSON.stringify(
					GridManager.GetGridById(gridID).dataSource.updateAddedRowKey(currentRowId, newKey)
				);
			},
		});
	...
	export function UpdateStartingRowHeader(gridID: string, startIndex: number): string {
		...
		const result = Auxiliary.CreateApiResponse({
			gridID,
			errorCode: OSFramework.DataGrid.Enum.ErrorCodes.API_FailedUpdateStartingRowHeader,
			callback: () => {
				GridManager.GetGridById(gridID).features.autoRowNumber.setStartIndex(startIndex);
			},
		});
	...
	export function SetValidationStatusByKey(
		...
		const result = Auxiliary.CreateApiResponse({
			...
			callback: () => {
				GridManager.GetGridById(gridID).features.validationMark.setCellStatusByKey(
					rowKey,
					columnID,
					isValid,
					errorMessage
	...
	export function ToggleRowDragging(gridID: string, allowRowDragging: boolean): string {
		...
		const result = Auxiliary.CreateApiResponse({
			gridID,
			errorCode: OSFramework.DataGrid.Enum.ErrorCodes.API_FailedToggleRowDragging,
			callback: () => {
				GridManager.GetGridById(gridID).features.gridReorder.toggleRowDragging(allowRowDragging);
			},
		});
...
namespace GridAPI.Rows {
	...
	export function RemoveClass(gridID: string, rowNumber: number, className: string): string {
		...
		return OutSystems.GridAPI.Rows.RemoveClass(gridID, rowNumber, className);
	}
	...
	export function RemoveRowsByNumberOrKey(gridID: string, rowNumbers: string, rowKeys: string): string {
		...
		return OutSystems.GridAPI.Rows.RemoveRowsByNumberOrKey(gridID, rowNumbers, rowKeys);
	}
	...
	export function UpdateAddedRowKey(gridID: string, currentRowId: string, newKey: string): string {
		...
		return OutSystems.GridAPI.Rows.UpdateAddedRowKey(gridID, currentRowId, newKey);
	}
	...
	export function UpdateStartingRowHeader(gridID: string, startIndex: number): string {
		...
		return OutSystems.GridAPI.Rows.UpdateStartingRowHeader(gridID, startIndex);
	}
	...
	export function SetValidationStatusByKey(
		...
		return OutSystems.GridAPI.Rows.SetValidationStatusByKey(gridID, rowKey, columnID, isValid, errorMessage);
	}
	...
	export function ToggleRowDragging(gridID: string, allowRowDragging: boolean): string {
		...
		return OutSystems.GridAPI.Rows.ToggleRowDragging(gridID, allowRowDragging);
	}
}
