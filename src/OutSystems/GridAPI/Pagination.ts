namespace OutSystems.GridAPI.Pagination {
	...
	export function CreatePageButtons(gridID: string, phID: string, buttonQuantity: number): void {
		GridManager.Events.Subscribe(
			...
			(_gridId: string, gridObj: OSFramework.DataGrid.Grid.IGrid) => {
				...
				gridObj.features.pagination.createPageButtons(phID, buttonQuantity);
	...
	export function GetCurrentPage(gridID: string): string {
		...
		const result = Auxiliary.CreateApiResponse({
			gridID,
			errorCode: OSFramework.DataGrid.Enum.ErrorCodes.API_FailedPaginationGetCurrentPage,
			callback: () => {
				const grid = GridManager.GetGridById(gridID);
	...
	export function MoveToPage(gridID: string, n: number): string {
		...
		const result = Auxiliary.CreateApiResponse({
			gridID,
			errorCode: OSFramework.DataGrid.Enum.ErrorCodes.API_FailedPaginationSetCurrentPage,
			callback: () => {
				const grid = GridManager.GetGridById(gridID);

				// we don't want to set page index if there is server side pagination
				if (grid.config.serverSidePagination) {
					throw new Error(OSFramework.DataGrid.Enum.ErrorMessages.SetCurrentPageServerSidePagination);
				}

				if (!grid.features.pagination.moveToPage(n)) {
					throw new Error(OSFramework.DataGrid.Enum.ErrorMessages.SetCurrentPage);
				}
			},
		});
	...
	export function RegisterCurrentPageLabel(gridID: string, phID: string): void {
		GridManager.Events.Subscribe(
			...
			(_gridId: string, gridObj: OSFramework.DataGrid.Grid.IGrid) => {
				...
				gridObj.features.pagination.registerLabel(OSFramework.DataGrid.Enum.PageLabel.PageIndex, phID);
	...
	export function RegisterPageCountLabel(gridID: string, phID: string): void {
		GridManager.Events.Subscribe(
			...
			(_gridId: string, gridObj: OSFramework.DataGrid.Grid.IGrid) => {
				...
				gridObj.features.pagination.registerLabel(OSFramework.DataGrid.Enum.PageLabel.PageCount, phID);
	...
	export function RegisterPageSizeLabel(gridID: string, phID: string): void {
		GridManager.Events.Subscribe(
			...
			(_gridId: string, gridObj: OSFramework.DataGrid.Grid.IGrid) => {
				...
				gridObj.features.pagination.registerLabel(OSFramework.DataGrid.Enum.PageLabel.PageSize, phID);
	...
	export function RegisterRowEndLabel(gridID: string, phID: string): void {
		GridManager.Events.Subscribe(
			...
			(_gridId: string, gridObj: OSFramework.DataGrid.Grid.IGrid) => {
				...
				gridObj.features.pagination.registerLabel(OSFramework.DataGrid.Enum.PageLabel.RowEnd, phID);
	...
	export function RegisterRowStartLabel(gridID: string, phID: string): void {
		GridManager.Events.Subscribe(
			...
			(_gridId: string, gridObj: OSFramework.DataGrid.Grid.IGrid) => {
				...
				gridObj.features.pagination.registerLabel(OSFramework.DataGrid.Enum.PageLabel.RowStart, phID);
	...
	export function RegisterRowTotalLabel(gridID: string, phID: string): void {
		GridManager.Events.Subscribe(
			...
			(_gridId: string, gridObj: OSFramework.DataGrid.Grid.IGrid) => {
				...
				gridObj.features.pagination.registerLabel(OSFramework.DataGrid.Enum.PageLabel.RowTotal, phID);
...
namespace GridAPI.Pagination {
	...
	export function CreatePageButtons(gridID: string, phID: string, buttonQuantity: number): void {
		...
		return OutSystems.GridAPI.Pagination.CreatePageButtons(gridID, phID, buttonQuantity);
	}
	...
	export function RegisterCurrentPageLabel(gridID: string, phID: string): void {
		...
		return OutSystems.GridAPI.Pagination.RegisterCurrentPageLabel(gridID, phID);
	}
	...
	export function RegisterPageCountLabel(gridID: string, phID: string): void {
		...
		return OutSystems.GridAPI.Pagination.RegisterPageCountLabel(gridID, phID);
	}
	...
	export function RegisterPageSizeLabel(gridID: string, phID: string): void {
		...
		return OutSystems.GridAPI.Pagination.RegisterPageSizeLabel(gridID, phID);
	}
	...
	export function RegisterRowStartLabel(gridID: string, phID: string): void {
		...
		return OutSystems.GridAPI.Pagination.RegisterRowStartLabel(gridID, phID);
	}
	...
	export function RegisterRowTotalLabel(gridID: string, phID: string): void {
		...
		return OutSystems.GridAPI.Pagination.RegisterRowTotalLabel(gridID, phID);
	}
}
