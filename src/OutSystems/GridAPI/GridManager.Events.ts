namespace OutSystems.GridAPI.GridManager.Events {
	...
	export function Unsubscribe(
		...
		if (grid !== undefined) {
			...
			if (_pendingEvents.has(gridID)) {
				const index = _pendingEvents.get(gridID).findIndex((element) => {
...
namespace GridAPI.GridManager.Events {
	...
	export function Subscribe(
		...
		return OutSystems.GridAPI.GridManager.Events.Subscribe(gridID, eventName, callback);
	}
	...
	export function Unsubscribe(
		...
		return OutSystems.GridAPI.GridManager.Events.Unsubscribe(gridID, eventName, callback);
	}
}
