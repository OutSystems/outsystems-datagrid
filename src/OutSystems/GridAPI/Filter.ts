namespace OutSystems.GridAPI.Filter {
	...
	export function Search(gridID: string, searchedValue: string): string {
		...
		const result = Auxiliary.CreateApiResponse({
			gridID,
			errorCode: OSFramework.DataGrid.Enum.ErrorCodes.API_FailedFilterSearch,
			callback: () => {
				const grid = GridManager.GetGridById(gridID);
				...
				grid.gridEvents.trigger(OSFramework.DataGrid.Event.Grid.GridEventType.SearchEnded, grid);
			},
		});
	...
	export function Activate(gridID: string, columnID: string): string {
		...
		const result = Auxiliary.CreateApiResponse({
			...
			callback: () => {
				GridManager.GetGridById(gridID).features.filter.activate(columnID);
			},
		});
	...
	export function Deactivate(gridID: string, columnID: string): string {
		...
		const result = Auxiliary.CreateApiResponse({
			...
			callback: () => {
				GridManager.GetGridById(gridID).features.filter.deactivate(columnID);
			},
		});
	...
	export function SetColumnFilterOptions(
		...
		const result = Auxiliary.CreateApiResponse({
			...
			callback: () => {
				GridManager.GetGridById(gridID).features.filter.setColumnFilterOptions(
					columnID,
					JSON.parse(options),
					maxVisibleOptions
				);
...
namespace GridAPI.Filter {
	...
	export function SetColumnFilterOptions(
		...
		return OutSystems.GridAPI.Filter.SetColumnFilterOptions(gridID, columnID, options, maxVisibleOptions);
	}
}
