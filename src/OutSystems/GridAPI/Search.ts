	...
	export function SearchData(
		...
		GridManager.Events.Subscribe(
			...
			(_gridId: string, gridObj: OSFramework.DataGrid.Grid.IGrid) => {
				gridObj.features.search.searchData(searchID, promptMessage, highlightResults);
			}
		);

		Performance.SetMark('Search.search-end');
		Performance.GetMeasure('@datagrid-Search.search-data', 'Search.search-data', 'Search.search-end');
	}
}
