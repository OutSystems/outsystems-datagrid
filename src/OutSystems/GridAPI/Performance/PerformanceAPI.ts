...
namespace OutSystems.GridAPI.Performance {
	...
	export function GetAllMeasures(keyword = ''): PerformanceEntryList {
		...
		return performanceObj.getEntriesByType('measure').filter((rec) => regex.test(rec.name));
	}
	...
	export function GetDuration(name: string): number {
		const measure: PerformanceEntry = performanceObj.getEntriesByName(name)[0];
	 ...
	 */
	export function GetMeasure(name: string, key1: string, key2: string): PerformanceEntry {
