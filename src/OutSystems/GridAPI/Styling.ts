namespace OutSystems.GridAPI.Styling {
	...
	export function SetCellCssClass(gridID: string, columnID: string, rowIndex: number, className: string): string {
		...
		const result = Auxiliary.CreateApiResponse({
			...
			callback: () => {
				...
				GridManager.GetGridById(gridID).features.cellStyle.addClass(binding, rowIndex, className, true);
			},
		});
	...
	export function SetColumnCssClass(
		...
		const result = Auxiliary.CreateApiResponse({
			...
			callback: () => {
				GridManager.GetGridById(gridID).features.styling.addColumnCssClass(columnID, cssClass, applyToHeader);
			},
		});
	...
	export function RemoveColumnCssClass(gridID: string, columnID: string, cssClass: string): string {
		...
		const result = Auxiliary.CreateApiResponse({
			...
			callback: () => {
				GridManager.GetGridById(gridID).features.styling.removeColumnCssClass(columnID, cssClass);
			},
		});
...
namespace GridAPI.Styling {
	...
	export function SetCellCssClass(gridID: string, columnID: string, rowIndex: number, className: string): string {
		...
		return OutSystems.GridAPI.Styling.SetCellCssClass(gridID, columnID, rowIndex, className);
	}
	...
	export function SetColumnCssClass(
		...
		return OutSystems.GridAPI.Styling.SetColumnCssClass(gridID, columnID, cssClass, applyToHeader);
	}
	...
	export function RemoveAllCssClassesFromCell(gridID: string, columnID: string, rowIndex: number): string {
		...
		return OutSystems.GridAPI.Styling.RemoveAllCssClassesFromCell(gridID, columnID, rowIndex);
	}
	...
	export function RemoveColumnCssClass(gridID: string, columnID: string, cssClass: string): string {
		...
		return OutSystems.GridAPI.Styling.RemoveColumnCssClass(gridID, columnID, cssClass);
	}
	...
	export function SetColumnWordWrap(
		...
		return OutSystems.GridAPI.Styling.SetColumnWordWrap(gridID, columnID, wordWrapValue, dynamicHeight);
	}
}
