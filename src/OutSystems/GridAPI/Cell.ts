namespace OutSystems.GridAPI.Cells {
	...
	export function SetValidationStatus(
		...
		const result = Auxiliary.CreateApiResponse({
			gridID,
			errorCode: OSFramework.DataGrid.Enum.ErrorCodes.API_FailedSetValidationStatus,
			callback: () => {
	...
	export function ValidateRow(gridID: string, rowIndex: number): string {
		...
		const result = Auxiliary.CreateApiResponse({
			gridID,
			errorCode: OSFramework.DataGrid.Enum.ErrorCodes.API_FailedApplyRowValidation,
			callback: () => {
				GridManager.GetGridById(gridID).features.validationMark.validateRow(rowIndex);
			},
		});
	...
	export function SetCellData(
		...
		const result = Auxiliary.CreateApiResponse({
			gridID,
			errorCode: OSFramework.DataGrid.Enum.ErrorCodes.API_FailedSetCellData,
			callback: () => {
				const grid = GridManager.GetGridById(gridID);
				const column = grid.getColumn(columnID);
...
namespace GridAPI.Cells {
	...
	export function SetValidationStatus(
		...
		return OutSystems.GridAPI.Cells.SetValidationStatus(gridID, rowIndex, columnID, isValid, errorMessage);
	}
	...
	export function ValidateCell(
		...
		return OutSystems.GridAPI.Cells.ValidateCell(gridID, rowIndex, columnID, triggerOnCellValueChange);
	}
