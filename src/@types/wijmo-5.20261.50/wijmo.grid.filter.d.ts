...
declare module wijmo.grid.filter {
    const ColumnFilterEditorClsNames: {
        filterType: string;
        hostElement: string;
        sortButtons: string;
        filterHeader: string;
    };
    const ConditionFilterEditorClsNames: {
        hostElement: string;
    };
    const FlexGridFilterClsNames: {
        elemFilter: string;
        filter: string;
        filterOff: string;
        filterOn: string;
        hostElement: string;
        filterTopBorder: string;
    };
    const ValueFilterEditorClsNames: {
        hostElement: string;
...
declare module wijmo.grid.filter {
    ...
    /**
     * Specifies mode of column filter.
     */
    enum FilterMode {
        /** single filter mode that only one filter type can be selected from value filter type or condition filter type. */
        SingleFilter = 0,
        /** multiple filters mode that apply both value filter type and condition filter type. */
        MultiFilter = 1
    }
    /**
    ...
    class FlexGridFilter {
        ...
        /**
         * Gets or sets the default filter mode to use.
         *
         * This value can be overridden in filter mode for specific columns.
         * For example, the code below creates a filter that filter mode by
         * single filter mode on all columns except the "ByMultiFilter" column:
         *
         * ```typescript
         * import { FlexGridFilter, FilterMode } from '@mescius/wijmo.grid.filter';
         * let filter = new FlexGridFilter(flex);
         * filter.defaultFilterMode = FilterMode.SingleFilter;
         * let col = flex.getColumn('ByMultiFilter'),
         *     cf = filter.getColumnFilter(col);
         ...
         */
        onExclusiveValueSearchChanged(e?: wijmo.EventArgs): void;
        /**
         * Occurs after the showIncludeSelectionBtn property is changed.
         */
        readonly _showIncludeSelectionChanged: Event<FlexGridFilter, EventArgs>;
        /**
         * Raises the {@link showIncludeSelectionChanged} event.
         */
        _onShowIncludeSelectionChanged(e?: wijmo.EventArgs): void;
        _asColumn(col: wijmo.grid.Column | string | number): wijmo.grid.Column;
        private _filter;
        private _adjustTabPanelStyle;
        private _formatItem;
        ...
        _removeFilter(index: number): void;
        _adjustBindingColumns(_columnCollection: wijmo.grid.ColumnCollection, e: wijmo.collections.NotifyCollectionChangedEventArgs): void;
    }
}
declare module wijmo.grid.filter {
    /**
...
declare module wijmo.grid.filter {
    ...
    class ColumnFilter implements IColumnFilter {
         ...
         *
         * Setting this property to null causes the filter mode to use the value
         * defined by the owner filter's {@link FlexGridFilter.defaultFilterMode}
         * property.
         */
        ...
        _setColumn(column: wijmo.grid.Column): void;
        /**
         * Gets or sets the aria-label attribute of {@link ColumnFilterEditor} host element.
         *
         * The default value for this property is "Filter Editor for Column".
         */
        ariaLabel: string;
        /**
...
declare module wijmo.grid.filter {
    ...
    class ValueFilter implements IColumnFilter {
        ...
        readonly isActive: boolean;
        /**
         * Gets or sets the custom filter function for the ValueFilter.
         *
         * This function allows fully custom filtering logic to be applied to each data item.
         * The default value for this property is **null** (no custom filter is applied)
         */
        customFilter: ICustomFilter | null;
        /**
         * Gets a value that indicates whether a value passes the filter.
         *
         * @param value The value to test.
         ...
         */
        implementsInterface(interfaceName: string): boolean;
        setColumnFilter(columnFilter: ColumnFilter): void;
        updateShowStatus(values: any): any;
        setInitialAllValues(): void;
    }
    /**
     * Represents a custom filtering function.
     * @param filterText The current text entered in the filter editor (search box) of the ValueFilter.
     * @param filterItem The list item being evaluated, which is ICustomFilterItem object, contains: value, text, and show properties.
     * @returns It should return **true** to include the item in the filtered results, or **false** to exclude it. If any other value is returned, we will implicitly cast the return value.
     */
    type ICustomFilter = (filterText: string, filterItem: ICustomFilterItem) => boolean;
    /**
     * Represents a filter item.
     * @param value The raw cell value from the data source.
     * @param text The formatted/display text shown in the list (used for matching by default).
     * @param show The current checked state of the item in the list (true = checked, false = unchecked).
     */
...
declare module wijmo.grid.filter {
    ...
    class ValueFilterEditor extends wijmo.Control {
        ...
        private _cbIncludeSel;
        private _spIncludeSel;
        protected _lblIncludeSel: HTMLLabelElement;
        private _divValues;
        protected _divOptions: HTMLElement;
        _lbValues: wijmo.input.ListBox;
        private _aExceeds;
        private _divExceeds;
        private _popup;
        static _ctrlTemplate: string;
        private _adjustWidthsTimer;
        private _updateFilterTimer;
        private _filterTextChangedBnd;
        private _cbSelectAllClickedBnd;
        /**
         * Gets or sets the template used to instantiate {@link ValueFilterEditor} controls.
         */
         ...
         */
        canApply: boolean;
        protected readonly _shouldIncludeSelection: boolean;
        /**
...
declare module wijmo.grid.filter {
    ...
    class ColumnFilterEditor extends wijmo.Control {
        ...
        private _tabPanel;
        static _ctrlTemplate: string;
        private _disallowedControls;
        private _btnClickedHandle;
        private _cbSelectAllClickedHandle;
        private _andOrChangedHandle;
        private _filterAppliedHandler;
        private _cmb1Timer;
        private _cmb2Timer;
        private _val1Timer;
        private _val2Timer;
        private _andOrTimer;
        private _isConditionFilterChanged;
        /**
         * Gets or sets the template used to instantiate {@link ColumnFilterEditor} controls.
         */
        static controlTemplate: string;
        static readonly _multiFilterTemplate: string;
        static readonly _singleFilterTemplate: string;
        static isCustomedTemplate(template: string): boolean;
        /**
        ...
        private getActiveFilterType;
        private raiseFilterChangedHandler;
        private bindEventsForMultiFilter;
        private updateConditionEditorAndTirggerFilterChanged;
        private _cbSelectAllClicked;
        private _andOrChanged;
