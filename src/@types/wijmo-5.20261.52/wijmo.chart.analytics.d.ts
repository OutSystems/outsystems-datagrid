...
declare module wijmo.chart.analytics {
    ...
    class FunctionSeries extends TrendLineBase {
        ...
        _calculateX(_value: number): number;
        _calculateY(_value: number): number;
    }
    /**
...
declare module wijmo.chart.analytics {
    ...
    class ErrorBar extends wijmo.chart.Series {
        ...
        getDataRect(currentRect?: wijmo.Rect, _calculatedRect?: wijmo.Rect): wijmo.Rect;
...
declare module wijmo.chart.analytics {
    ...
    /**
     * Represents the final statistics for one rendered box in a
     * {@link BoxWhisker} series.
     *
     * When the box plot values have already been calculated and
    ...
    class BoxWhisker extends wijmo.chart.SeriesBase {
        ...
        private _boxPlotData;
        private _boxPlotDataMap;
        ...
        /**
         * Gets or sets an array of pre-processed box plot statistics.
         *
         * Use this property when the min/max/quartile/median/mean values for each
         * box have already been calculated in advance, for example by a backend
         * service or during an earlier preprocessing step.
         *
         * Each entry corresponds to the point at the same index in the bound raw
         * array data. When a valid {@link IBoxPlotData} entry is provided, the
         * series renders that box from the supplied statistics instead of
         * recalculating them from the raw sample array. When an entry is missing
         * or invalid, the series falls back to the default calculation path for
         * that point.
         *
         * The supplied values are treated as the final box plot result for that
         * point. In particular, min/max/outlierPoints/innerPoints are used as-is
         * and are not recomputed from the raw sample array.
         *
         * When the series uses CollectionView-backed data, including arrays that
         * are internally wrapped as a {@link CollectionView}, the processed
         * statistics are associated with the original source items by item identity
         * and resolved against the current view items during rendering. This keeps
         * the processed values aligned when the view order changes because of
         * sorting, filtering, or paging.
         */
        boxPlotData: IBoxPlotData[];
        private _rendering;
        ...
        private _getProcessedBoxPlotData;
        private _getBoxPlotDataSourceItems;
        private _getBoxPlotDataViewItems;
    }
    class _BoxPlot {
        private _data;
        private _min;
        ...
        private _showOutliers;
        constructor(data: number[] | IBoxPlotData, quartileCalculation: QuartileCalculation, showOutliers: boolean);
        ...
        _parse(): void;
        private _applyProcessedData;
        _quartileInc(data: number[], percent: number): number;
        _quartileExc(data: number[], percent: number): number;
    }
}
declare module wijmo.chart.analytics {
}
