...
declare module wijmo.collections {
    ...
    interface IEditableCollectionView<T = any> extends ICollectionView {
        ...
        /**
         * Gets or sets the zero-based index that the new row template uses when
         * inserting items.
         *
         * When paging is enabled, values in the range `0..pageSize - 1` map to
         * positions within the current page. Values less than zero or greater than
         * or equal to the current page size keep the original add-new behavior and
         * append items to the end of the source collection. When paging is disabled,
         * non-negative values are treated as positions within the full source
         * collection.
         */
        newItemIndex: number;
        /**
         ...
         *
         *
         * Similar to removeAt, the index is relative to the view, not to the source collection.
         * When paging is enabled, values in the range `0..pageSize - 1` map to
         * positions within the current page. Values less than zero or greater than
         * or equal to the current page size are ignored. When sorting , filtering or grouping
         * is active, valid indexes fall back to the original add-new behavior and
         * append items to the end of the source collection. When paging is disabled,
         * non-negative values are treated as positions within the full source
         * collection.
         *
         * @param index Zero-based position where the new item should be inserted.
         * @param item Optional item to add.
         * @param commit Whether to commit the new item immediately.
         * @return The item that was added to the collection, or null if the operation failed.
         */
...
declare module wijmo {
    const ClipboardClsNames: {
        clipboard: string;
    };
    const ControlClsNames: {
        content: string;
        hostElement: string;
        template: string;
    };
    const ControlStateClsNames: {
        active: string;
        checked: string;
        collapsed: string;
        collapsing: string;
    ...
    const InputFormElementsClsNames: {
        btn: string;
        btnDefault: string;
        btnGroup: string;
    ...
    const PrintDocumentClsNames: {
        printDocument: string;
    };
    const TooltipClsNames: {
        hostElement: string;
    };
    const UtilitesClsNames: {
        align: string;
        alignVCenter: string;
        animated: string;
        centerVert: string;
...
declare module wijmo {
    ...
    function _isPlainArray(value: any): boolean;
    /**
     ...
     */
    function assert(condition: boolean, msg: string, showStackTrace?: boolean): void;
    /**
     ...
     */
    function _warning(msg: string): void;
    /**
    ...
    function _getFocusableElements(parent: HTMLElement, backward?: boolean): any[];
    /**
    ...
    function _getDevicePixelTolerance(base: number, roundFn?: (value: number) => number): number;
    function _getElementBorderWidth(element: HTMLElement | null, side: 'left' | 'right' | 'top' | 'bottom'): number;
    function _copyProps(src: any, dst: any, dstProps: string[], excludeProps?: string[]): void;
}
declare module wijmo {
    /**
     * Represents a color.
...
declare module wijmo {
    ...
    class Globalize {
        ...
        private static _toExponentialStr;
...
declare module wijmo {
    ...
    enum Aggregate {
        ...
        Last = 13,
        /**
         * Returns the count of numeric values in the group (excluding nulls and NaN).
         */
        CntNums = 14
    }
    /**
...
declare module wijmo.collections {
    ...
    class CollectionView<T = any> implements IEditableCollectionView, IPagedCollectionView {
         ...
         */
        endUpdate(force?: boolean): void;
        _stopTrackChanges(): void;
        _restoreTrackChanges(): void;
        /**
         * Gets a value that indicates whether notifications are currently suspended
         * (see {@link beginUpdate} and {@link endUpdate}).
         */
        ...
        /**
         * Gets or sets the zero-based index that {@link FlexGrid}'s new row template
         * uses when inserting new items.
         *
         * The index is honored only when sorting, filtering, and grouping are inactive.
         * With paging enabled, values in the range `0..pageSize - 1` map to positions
         * within the current page; values less than zero or greater than or equal to the
         * current {@link pageSize} keep the original {@link addNew} behavior and append
         * the item to the end of the {@link sourceCollection}. When paging is disabled,
         * non-negative values are treated as positions within the full
         * {@link sourceCollection}.
         *
         * @example
         * // Use the original addNew behavior (append to sourceCollection)
         * cv.newItemIndex = -1;
         *
         ...
         * Adds a new item to the collection at the specified position.
         *
         * This method is similar to {@link addNew}, but allows you to control where the new
         * item is inserted. With paging enabled, values in the range `0..pageSize - 1`
         * map to positions within the current page. Values less than zero or greater than
         * or equal to the current {@link pageSize} are ignored. When sorting, filtering,
         * or grouping is active, valid indexes fall back to the original {@link addNew}
         * behavior and append items to the end of the {@link sourceCollection}. When paging is disabled,
         * non-negative values are treated as positions within the full
         * {@link sourceCollection}.
         *
         * @param index Zero-based position where the new item should be inserted.
         * @param item Item to be added to the collection (optional).
         * @param commit Whether to commit the new item immediately.
         * @return The item that was added to the collection, or null if the transaction failed.
         */
        insertAt(index: number, item?: Partial<T> | T, commit?: boolean): Partial<T> | T;
        _insertNewItem(index: number, item?: Partial<T> | T, commit?: boolean, addNewFallback?: boolean): Partial<T> | T;
        _createNewItem(item?: Partial<T> | T): Partial<T> | T;
        _isValidInsertIndex(index: number): boolean;
        _isPaged(): boolean;
        _hasActiveViewTransform(): boolean;
        _isSort(): boolean;
        _isFilter(): boolean;
        _isGroup(): boolean;
        _getInsertAtPosition(index: number): number;
        _finalizeNewItem(item: Partial<T> | T, commit: boolean): void;
        /**
         * Ends the current add transaction and saves the pending new item.
         */
        commitNew(): void;
    ...
    class _CollectionViewItemMap<TValue = any> {
        private _map;
        readonly hasValues: boolean;
        clear(): void;
        reset(items: any[], values: TValue[], validator?: (value: TValue) => boolean): void;
        getValue(item: any): TValue;
    }
    class _NullValue {
        _id: number;
...
declare module wijmo {
    ...
    class Control extends ControlBase {
        ...
        readonly isEventsSuspended: boolean;
        /**
         * Sets the focus to this control.
         */
        focus(): void;
        _internalFocus(options?: IFocusOptions): void;
        /**
...
declare module wijmo {
    ...
    function updatePopupPosition(popup: HTMLElement, ref: any, position?: PopupPosition | Boolean): void;
...
declare module wijmo {
    ...
    function _checkLicenseVersion(vLc: string, vWj: string, nVP: string): boolean;
    function _ckHT(host: any, domain: any): any;
    function _isBeforeVersion(vLc: any): boolean;
    function _isOldLicense(lic: any): boolean;
    function isSameOrigin(loc1: any, loc2: any): boolean;
}
